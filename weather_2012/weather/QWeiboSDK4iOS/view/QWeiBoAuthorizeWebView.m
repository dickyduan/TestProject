...
#import "QWeiboAuthorizeWebView.h"
#import <QuartzCore/QuartzCore.h> 

@interface QWeiboAuthorizeWebView (Private)
...
@implementation QWeiboAuthorizeWebView

@synthesize delegate;

#pragma mark - QWeiboAuthorizeWebView Life Circle

- (id)init
{
    if (self = [super initWithFrame:CGRectMake(0, 00, 320, 361)])
    {
        // background settings
...
//        panelView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 411)];
//
...
//        [[containerView layer] setBorderWidth:1.0];
        ...
        // add the web view
        webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 20, 320, 361)];
		[webView setDelegate:self];
//		[containerView addSubview:webView];
        [(UIScrollView *)[[webView subviews] objectAtIndex:0] setBounces:NO];
//        [(UIScrollView *)[[webView subviews] objectAtIndex:0] setContentOffset:CGPointMake(0, 40)];
        [self addSubview:webView];
        
        closeButton = [[UIButton alloc] initWithFrame:CGRectMake(300, 0, 19, 19)];
        [closeButton setBackgroundImage:[UIImage imageNamed:@"close_btn"] forState:UIControlStateNormal];
...
- (void)bounceOutAnimationStopped
{
    ...
    [webView setAlpha:0.8];
	[webView setTransform:CGAffineTransformScale(CGAffineTransformIdentity, 0.9, 0.9)];
...
- (void)bounceInAnimationStopped
{
	...
	[webView setTransform:CGAffineTransformScale(CGAffineTransformIdentity, 1.0, 1.0)];
	[UIView commitAnimations];
}
...
#pragma mark - QWeiboAuthorizeWebView Public Methods
...
- (BOOL)webView:(UIWebView *)aWebView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    if (range.location != NSNotFound)
    {
        ...
        NSString * verify  = [request.URL.absoluteString substringFromIndex:range.location + range.length];
        if ([delegate respondsToSelector:@selector(authorizeWebView:didReceiveAuthorizeCode:)])
        {
            [delegate authorizeWebView:self didReceiveAuthorizeCode:verify];
        }
    }
