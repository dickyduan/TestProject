...
//		NSString *filePath = [files objectForKey:key];
        id file = [files objectForKey:key];
        NSData *fileData = nil;
        NSString *header = nil;
        if([file isKindOfClass:[UIImage class]])
        {
            fileData = UIImagePNGRepresentation(file);
            header = [NSString stringWithFormat:headerTemplate, key, @"image.png"];
        }
        else if([file isKindOfClass:[NSString class]])
        {
            fileData = [NSData dataWithContentsOfFile:file];
            header = [NSString stringWithFormat:headerTemplate, key, [[file componentsSeparatedByString:@"/"] lastObject]];
        }
